...
class GrowingAudioFile {
  ...
  GrowingAudioFile({required this.filePath}) : _pcmLength = 0, _lastEncodedChunkIndex = -1;
  ...
  Future<void> addChunk(Float32List samples, int chunkIndex) async {
    ...
    if (mp3Bytes.isNotEmpty) {
      await File(filePath).writeAsBytes(mp3Bytes, mode: FileMode.append, flush: true);
    }
  ...
  Future<void> finalize() async {
    ...
    if (finalBytes.isNotEmpty) {
      await File(filePath).writeAsBytes(finalBytes, mode: FileMode.append, flush: true);
    }
  ...
  void _writeMeta() {
    final meta = jsonEncode({"lastChunkIndex": _lastEncodedChunkIndex, "pcmLength": _pcmLength});
