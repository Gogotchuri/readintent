...
void main() {
  ...
  group("AudioGenerator - Generation Flow", () {
    test(
      "generate() with 3 chunks calls pipeline runInference 3 times",
      () async {
    ...
    test(
      "generate() accumulates audio in GrowingAudioFile - MP3 file grows on disk",
      () async {
    ...
    test(
      "generate() emits TTSSessionState with increasing bufferedDuration",
      () async {
        final factory = fakePipelineFactory(defaultAudioProducer);
        final gen = AudioGenerator(
          article: _makeArticle(3),
        ...
        final durations = states
            .map((s) => s.bufferedDuration.inMilliseconds)
            .toList();
    ...
    test(
      "generate() emits estimatedDuration proportional to tokens processed",
      () async {
        final factory = fakePipelineFactory(defaultAudioProducer);
        final gen = AudioGenerator(
        ...
        final withEstimate = states
            .where((s) => s.estimatedDuration != null)
            .toList();
    ...
    test(
      "generate() calls onBufferReady when buffered audio exceeds threshold",
      () async {
    ...
    test(
      "generate() calls onBufferReady after all chunks if threshold never reached",
      () async {
    ...
    test(
      "generate() finalizes audio file on completion - .meta deleted, MP3 exists",
      () async {
        final factory = fakePipelineFactory(defaultAudioProducer);
        final gen = AudioGenerator(
    ...
    test(
      "generate() emits isComplete=true on completion with final duration",
      () async {
        final factory = fakePipelineFactory(defaultAudioProducer);
        final gen = AudioGenerator(
          article: _makeArticle(2),
    ...
    test(
      "generate() emits error state on pipeline failure and rethrows",
      () async {
        final factory = fakePipelineFactory((chunk) {
  ...
  group("AudioGenerator - Cache Integration", () {
    test(
      "checkCacheForComplete() returns path when complete cached file exists",
      () async {
        final factory = fakePipelineFactory(defaultAudioProducer);
        final gen = AudioGenerator(
    ...
    test(
      "checkCacheForComplete() returns null when .meta exists (interrupted)",
      () async {
        final factory = fakePipelineFactory(defaultAudioProducer);
        final gen = AudioGenerator(
          article: _makeArticle(2),
        ...
        File(
          "$cachePath.meta",
        ).writeAsStringSync('{"lastChunkIndex":0,"pcmLength":1000}');
  ...
  group("AudioGenerator - Resume", () {
    test(
      "generate() resumes from correct chunk index when .meta file exists",
      () async {
        int inferenceCount = 0;
        final factory = fakePipelineFactory((chunk) {
          inferenceCount++;
        ...
        final gen1 = AudioGenerator(
          ...
          pipelineFactory: fakePipelineFactory(
            (chunk) => defaultAudioProducer(chunk),
          ),
        );
    ...
    test("generate() on resume calls onBufferReady immediately", () async {
      ...
      // Write partial state
      File(cachePath).writeAsBytesSync(
        Uint8List.fromList([0xFF, 0xFB, 0x90, 0x00]),
      ); // minimal mp3 header
      File(
        "$cachePath.meta",
      ).writeAsStringSync('{"lastChunkIndex":0,"pcmLength":24000}');
      ...
      final factory2 = fakePipelineFactory((chunk) {
        ...
        if (inferenceCount == 1 && bufferReadyCalledBeforeGeneration) {
          // buffer ready was called before first inference on resume
        }
        return defaultAudioProducer(chunk);
      });
    ...
    test("generate() on resume preserves existing buffered duration", () async {
      ...
      File(
        cachePath,
      ).writeAsBytesSync(Uint8List.fromList([0xFF, 0xFB, 0x90, 0x00]));
      File(
        "$cachePath.meta",
      ).writeAsStringSync('{"lastChunkIndex":0,"pcmLength":24000}');
  ...
  group("AudioGenerator - Disposal", () {
    test(
      "dispose() stops generation mid-loop (respects _disposed flag)",
      () async {
        int inferenceCount = 0;
        final factory = fakePipelineFactory((chunk) {
          inferenceCount++;
  ...
  group("AudioGenerator - State Stream", () {
    test(
      "stateStream emits all expected transitions across full generation",
      () async {
        final factory = fakePipelineFactory(defaultAudioProducer);
        final gen = AudioGenerator(
          article: _makeArticle(3),
    ...
    test(
      "bufferedDurationStream emits granular updates from the audio file",
      () async {
        final factory = fakePipelineFactory(defaultAudioProducer);
        final gen = AudioGenerator(
          article: _makeArticle(3),
