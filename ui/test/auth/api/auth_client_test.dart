...
void main() {
  ...
  test('Proto types mapped to local Session model correctly', () async {
    // Login
    when(mockClient.passwordLogin(any)).thenAnswer(
      (_) async => auth_pb.PasswordLoginResponse(session: sessionMock),
    );
    ...
    // Registration
    when(mockClient.passwordRegistration(any)).thenAnswer(
      (_) async => auth_pb.PasswordRegistrationResponse(session: sessionMock),
    );

    final regSession = await authClient.passwordRegistration(
      'email',
      'password',
      'first',
      'last',
  ...
  test('By default any exceptions are converted to AuthExceptions', () async {
    ...
    when(mockClient.passwordRegistration(any)).thenThrow(ConnectException(Code.unauthenticated, 'Registration failed'));
    expect(
  ...
  test("Validation error produces when ConnectException has details of by google.rpc.BadRequest", () async {
    final badRequest = BadRequest(fieldViolations: [
